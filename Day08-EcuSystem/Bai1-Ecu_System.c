...
typedef struct {
    ...
    int         trang_thai;             /* 0 = 0ff, 1 = On, 2 = Loi*/
    void (*xu_ly)(void *self);          /*Funcdion poiter tu xu ly*/
}ECU_t;


void khoi_tao_ecu(ECU_t *e, int id, char *ten, float nhiet_do, 
                    int trang_thai, void(*xu_ly)(void *self) ){
    e->id = id;
    strcpy(e->ten, ten);
    e->nhiet_do = nhiet_do;
...
void thong_ke(ECU_t *arr, int n, int *so_ecu_on, int *so_ecu_loi,
                    float *nhiet_do_tb){
    int i = 0;
    float tong = 0.0f;
